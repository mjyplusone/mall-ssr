...
<div class="goodslist">
            ...
            <div class="goodsinfo">
                <ul class="goodsinfo-list clearfix">
                    ...
                    <li v-for="goods in goodsList" :key="goods.productId">
                        <div class="content">
                            <a href="/"><img v-lazy="'/' + goods.productImage" alt="" width="100%"></a>
                            <div class="info">
    ...
    <modal v-if="this.$store.state.loginUserName" :mdShow="cartMdShow" @closeModal="closeCartModal">
        <div class="modal-text">加入购物车成功!</div>
        <div class="modal-btn">
            <div class="btn-success left" @click="closeCartModal">继续购物</div>
            <nuxt-link class="btn-success right" tag="div" to="/shopcart">查看购物车</nuxt-link>
        </div>
    </modal>
    <modal v-if="!this.$store.state.loginUserName" :mdShow="cartMdShow" @closeModal="closeCartModal">
...
// import {checkLogin} from '../api/users.js';
...
export default {
    ...
    async asyncData () {
      ...
      let {status, result: { list }} = await getGoodsList(param);
      if (status === '0') {
          return {
              goodsList: list,
              busy: false,
