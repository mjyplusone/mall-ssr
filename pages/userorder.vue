<template>
  <div class="userorder">
      ...
      <div class="userorder-content">
          ...
          <p class="noorder" v-if="orderList.length === 0">您还没有订单，去<router-link tag="a" to="/">逛逛</router-link>吧</p>
          <ul class="order-list">
              <li v-for="order in orderList" :key="order.orderId">
                  <div class="del" @click="deleteOrder(order.orderId)">删除</div>
...
import {getUserOrder, deleteOrder} from '../api/users.js';
import {formatPrice} from '../assets/js/format.js';
